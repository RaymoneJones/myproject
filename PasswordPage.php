...
if($_POST["submit"] == "确认修改"){      //判断提交的按钮名称是否为“确认修改”
    if($_SESSION["userpassword"] == $_POST["OldPassword"]){       //输入的旧密码正确
        if(strlen($_POST["NewPassword1"])<9 || strlen($_POST["NewPassword1"])>18) {
            echo "<script>alert('密码长度只能为9到18位');window.location.href='ModifyPassword.php';</script>";
        }else if($_POST["NewPassword1"] == $_POST["NewPassword2"]){     //两次输入的新密码一致则提示修改成功并修改数据库
